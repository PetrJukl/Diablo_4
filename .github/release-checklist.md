...
- workflow `Build release installer` vytvoří publish výstup, sestaví `Inno Setup` installer `.exe` a založí GitHub release automaticky
- pokud release notes upravíš ručně po vydání, workflow `Update release manifest` znovu sesynchronizuje `update-manifest.json`
